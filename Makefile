SRV_SRC = irminStorageSrv.ml* serverConfig.ml* storage/irminStorage.ml* storage/storageMeta.ml
...
all: imaplet srv store read_store lmtp imaplet_proxy
...
store: $(STORAGE_SRC) serverConfig.ml*
...
lmtp: lmtp.ml lmtpConfig.ml* serverConfig.ml* regex.ml*
	ocamlbuild -use-ocamlfind -no-hygiene -tag thread -tag "syntax(camlp4o)" -package core,lwt,lwt.unix,lwt.syntax,extlib,str lmtp.native
